
export const dynamicComponentReferenceIncludes = [
    'dynamic_component'
]

export const heroReferenceIncludes = [
    'hero',
    'hero.cta.link'
]

export const teaserReferenceIncludes = [
    'components.teaser.cta.link'
]

export const textAndImageReferenceIncludes = [
    'components.text_and_image_carousel.carousel_items.cta.link'
]

export const imageCardsReferenceIncludes = [
    'components.card_collection.cards.cta.link'
]

export const featuredArticlesReferenceIncludes = [
    'featured_articles.articles'
]

export const navigationReferenceIncludes = [
    'main_navigation',
    'main_navigation.items.link',
    'main_navigation.items.mega_menu',
    'main_navigation.items.mega_menu.sections.link',
    'main_navigation.items.mega_menu.sections.links.link',
    'main_navigation.items.mega_menu.cta_group',
    'main_navigation.items.mega_menu.cta_group.call_to_action.internal_link'
]

export const footerReferenceIncludes = [
    'footer_navigation',
    'footer_navigation.sections.link',
    'footer_navigation.sections.links.link'
]

export const userFormReferenceIncludes = [
    'user_form'
]
export const userFormJsonRtePathIncludes = [
    'user_form.form.user_consent_text'
]


// ==============  JSON RTE PATHS   ==============
export const textJSONRtePaths = [
    'components.text.content'
]

export const footerJsonRtePathIncludes = [
    'footer_navigation.copyright_info',
    'footer_navigation.built_by'
]

export const articleJSONRtePathIncludes = [
    'content'
]

export const pagePropsReferenceIncludes = [
    'page_props'
]

export const pdpPageReferenceIncludes = [
    'media',
    'details',
    'marketing',
    

]

export const pgpPageReferenceIncludes = [
    'pdp_page'
    
]


// ==============  PAGE REFERENCE INCLUDES   ==============
export const pageEntryReferenceIncludes = [
    ...heroReferenceIncludes,
    ...pagePropsReferenceIncludes,
    ...pdpPageReferenceIncludes,
    ...pgpPageReferenceIncludes
]
